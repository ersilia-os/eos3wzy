...
def extract_pka_statistics_from_sdf(sdf_file, csv_file, no_protonation):
    ...
    #reads the SDF file into supplier, an iterable object containing molecules.
    supplier = Chem.SDMolSupplier(sdf_file)

    #data is initialized as an empty dictionary to store pKa values for each molecule.
    data = {}

    #iterate through each molecule in supplier
    for mol in supplier:
        if mol is None:
            continue
        mol_name = mol.GetProp('_Name') if mol.HasProp('_Name') else 'Unknown'

        #Checks if the molecule has the pka property
        if mol.HasProp('pka'):
            idx_list = mol.GetProp('idx').split(',')
            pka_list = mol.GetProp('pka').split(',')
            pkas = [float(pka.strip('tensor()')) for pka in pka_list]

            #adds the pkas list to the data dictionary. If the molecule already exists in data, it extends the list with the new pKa values
            if mol_name not in data:
                data[mol_name] = pkas
            else:
                data[mol_name].extend(pkas)

    #iterate through the data dictionary
    '''
    If pkas list is not empty, it calculates the statistics:
      min_pka: Minimum pKa value.
      avg_pka: Average pKa value.
      max_pka: Maximum pKa value.
      num_pkas: Number of pKa values.
    '''
    for mol_name, pkas in data.items():
        if pkas:
            min_pka = np.min(pkas)
            avg_pka = np.mean(pkas)
            max_pka = np.max(pkas)
            num_pkas = len(pkas)
        else:
            min_pka = avg_pka = max_pka = num_pkas = None
...
def main():
    # Parse arguments and run the main logic
    ...
    # Check if the SDF file was created successfully (if  protonatio/deprotonation site was found)
